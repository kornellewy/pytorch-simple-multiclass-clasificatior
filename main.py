...
class ClassificationTrainer(pl.LightningModule):
    def __init__(self, hparams = {
        ...
        folders_structure = {
            ...
            self.last_best_valid_error = 100000.0
            self.test_x = []
            self.test_losses = []
            self.test_true = []
            self.test_pred = []
    ...
    def _split_dataset_to_dataloaders_and_return_classes(self):
        dataset = datasets.ImageFolder(self.dataset_path, self.img_transforms)
    ...
    def train_dataloader(self):
        train_dataloader = torch.utils.data.DataLoader(self.train_set, sampler=ImbalancedDatasetSampler(self.train_set),
    ...
    def validation_step(self, batch, batch_nb):
        ...
        if loss < self.last_best_valid_error:
            ...
            self.save_model(loss.item(), accuracy, 'valid')
        self.save_metadata()
        return loss
    ...
    def test_epoch_end(self, outputs):
        test_img_paths = self.save_and_convert_test_images_to_paths(self.test_x)
        ...
        plot_confusion_matrix(conf_matrix, confusion_matrix_path, self.classes)
        ...
        self.save_model(loss, accuracy, 'test')
        self.save_metadata()
    ...
    def save_metadata(self):
        json_file_path = os.path.join(self.folders_structure['metadata_json_folder'], 'metadata.json')
        with open(json_file_path, 'w') as fp:
            json.dump(self.model_metadata, fp)
    ...
    def save_and_convert_test_images_to_paths(self, test_images):
        ...
        for batch_idx, test_image in enumerate(test_images):
            if test_image.shape[0] > 1:
                for image_idx, image in enumerate(test_image):
                    image = self._inv_normalize_tensor(image)
    ...
    def save_model(self, val_loss = 0.0, val_acc = 0.0, mode= 'valid'):
        model_name = mode+'_'+'loss_' + str(round(val_loss, 4)) + '_accuracy_' + str(round(val_acc, 4)) + '.pth'
...
if __name__ == '__main__':
    ...
    model = ClassificationTrainer(dataset_path=dataset_path)
    checkpoint_save_path = str(Path(__file__).parent / '')
    # checkpoint_path = 'klasyfikacja/cloth_type_classification2/checkpoints_staff/lightning_logs/version_0/checkpoints/epoch=8-step=2632.ckpt'
    trainer = Trainer(gpus=1,precision=16, auto_lr_find=True,
                    auto_scale_batch_size=True, benchmark=True,
                    max_epochs=10, default_root_dir=checkpoint_save_path,
